    ...
    m_network_style(networkStyle)
{
    ...
    connectionsCount = new GUIUtil::ClickableLabel(platformStyle);
...
void BitcoinGUI::createActions()
{
    ...
    overviewAction = new QAction(platformStyle->MultiStatesIcon(":/styles/theme2/app-icons/mywallet", PlatformStyle::NavBar), tr(" &My Wallet"), this);
    ...
    receiveCoinsAction = new QAction(platformStyle->MultiStatesIcon(":/icons/receiving_addresses", PlatformStyle::NavBar), tr(" &Receive"), this);
    ...
    statsPageAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_5));
    ...
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed SIN Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from clipboard..."), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed SIN Transaction from clipboard"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible SIN command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::setCurrentWallet(WalletModel* wallet_model)
{
    ...
    if (homeAction->isChecked()) walletFrame->hideTransactionWidget();
...
void BitcoinGUI::changeEvent(QEvent *e)
{
    if (e->type() == QEvent::PaletteChange) {
        overviewAction->setIcon(platformStyle->MultiStatesIcon(QStringLiteral(":/icons/overview")));
        sendCoinsAction->setIcon(platformStyle->SingleColorIcon(QStringLiteral(":/icons/send")));
        receiveCoinsAction->setIcon(platformStyle->SingleColorIcon(QStringLiteral(":/icons/receiving_addresses")));
        historyAction->setIcon(platformStyle->MultiStatesIcon(QStringLiteral(":/icons/history")));
    }
...
void BitcoinGUI::updateStakingIcon()
{
    ...
    if (walletModel->wallet().getLastStakeTime() &&
...
{
    createContextMenu();
    setToolTip(QObject::tr("Unit to show amounts in. Click to select another unit."));
