...
This guide describes how to build sind, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Sin Core from source.
...
### 4. Clone Sinovate repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Sin Core repository to a directory.
All build scripts and commands will run from this directory.

``` bash
git clone https://github.com/SINOVATEblockchain/sinovate/sinovate.git
```
...
It is not necessary to build wallet functionality to run `sind` or  `sin-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/SINOVATEblockchain/sinovate/blob/master/doc/descriptors.md), `sqlite` is required.
...
Sin Core includes a GUI built with the cross-platform Qt Framework.
...
You can deploy a `.dmg` containing the Sin Core application using `make deploy`.
...
There are many ways to configure Sin Core, here are a few common examples:
...
Run the following in your terminal to compile Sin Core:
...
Sin Core should now be available at `./src/sind`.
If you compiled support for the GUI, it should be available at `./src/qt/sin-qt`.

The first time you run `sind` or `sin-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/SIN/
```

Before running, you may create an empty configuration file:

```shell
mkdir -p "/Users/${USER}/Library/Application Support/SIN"

touch "/Users/${USER}/Library/Application Support/SIN/sin.conf"

chmod 600 "/Users/${USER}/Library/Application Support/SIN/sin.conf"
```

You can monitor the download process by looking at the debug.log file:

```shell
tail -f $HOME/Library/Application\ Support/SIN/debug.log
```

## Other commands:

```shell
./src/sind -daemon      # Starts the sin daemon.
./src/sin-cli --help    # Outputs a list of command-line options.
./src/sin-cli help      # Outputs a list of RPC commands when the daemon is running.
./src/qt/sin-qt -server # Starts the sin-qt server mode, allows sin-cli control
```
